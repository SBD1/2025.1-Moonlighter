...
def buscarNarracao(busca):
    ...
    if busca is None:
        cursor.execute("""SELECT D."conteudo", D."idDialogo"
                        ...
                        INNER JOIN "npc" N
                        ON N."idNPC" = DN."idNPC"
                        WHERE N."idNPC" = (SELECT "idNPC" FROM npc WHERE "nome" = 'Mundo') AND D."idDialogoPai" IS NULL AND D."tipo" = 'Final';""")
    else:
        cursor.execute("""SELECT D."conteudo", D."idDialogo"
                        FROM "dialogo" D 
                            INNER JOIN "dialogo_npc" DN 
                            ...
                            INNER JOIN "npc" N
                            ON N."idNPC" = DN."idNPC"
                        WHERE N."idNPC" = (SELECT "idNPC" FROM npc WHERE "nome" = 'Mundo') AND D."idDialogoPai" = %s AND D."tipo" = 'Final';""",
                        (busca,)
                    )
